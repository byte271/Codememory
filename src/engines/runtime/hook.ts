 ...
 *
 * v0.2: Both modes now support auto-instrumentation:
 *   - CJS: auto via Module._load monkey-patch (unchanged).
 *   - ESM: auto via Node 20.6+ `module.register()` loader hook (new).
 *         Falls back to manual observer API on older Node versions.
 *
...
export class InstrumentationHook {
  ...
  /**
   * ESM mode: logs instructions for manual instrumentation.
   * Node 20.6+ auto-instrumentation via `module.register()` is planned for
   * a future release after the loader hook infrastructure is fully built out.
   */
