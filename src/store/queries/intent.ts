...
export class IntentQueries {
  ...
  /**
   * Marks an intent as replaced (v0.2 lineage).
   * Called when capture_intent records a replacement with parent_intent_id.
   *
   * Uses a conditional UPDATE (WHERE status = 'active') to prevent a race
   * where two concurrent capture_intent calls both claim the same parent.
   * The first caller wins; the second sees zero affected rows.
   *
   * @param id The memory_id of the parent intent to mark replaced.
   * @returns True if the intent was marked replaced, false if it was already
   *          replaced by a concurrent caller.
   */
  public markReplaced(id: string): boolean {
    const result = this.manager.prepare(
      "UPDATE intent_records SET status = 'replaced' WHERE id = ? AND status = 'active'"
    ).run(id);
    return result.changes > 0;
  }

  /**
   * Returns the raw Database handle for use with transactions.
   * Needed by CaptureIntentTool to atomically insert + markReplaced.
   */
