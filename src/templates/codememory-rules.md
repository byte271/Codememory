...
The five Codememory tools registered via MCP are:
...
- `query_memory`   — look up prior intents / runs / failures
...
intent + code + recent runtime + recent failures in one structured payload.
Read it. Use it. It is the single source of truth for the fix.
